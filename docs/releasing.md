...
- `doorframe mcp` optional read-only MCP server
...
10. Run an npx smoke test:
   ...
   ```bash
   npx doorframe@0.1.15 demo
   npx doorframe@0.1.15 serve
   ```

11. Run a Docker smoke test:

    ```bash
    docker run --rm -p 3000:3000 -v doorframe-data:/data ghcr.io/vtboyarc/doorframe:0.1.15
    ```
