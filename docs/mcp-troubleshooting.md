...
```bash
npx -y doorframe@0.1.10 mcp --project /absolute/path/to/doorframe.sqlite --project-id project_123 --mode standard --max-results 25
```

Or run the built-in doctor with the same values from the MCP Setup page:

```bash
npx -y doorframe@0.1.10 mcp doctor --project /absolute/path/to/doorframe.sqlite --project-id project_123 --mode standard --max-results 25
```
...
- Use the generated `npx -y doorframe@0.1.10 mcp ...` command.
...
```json
{
  "mcpServers": {
    "doorframe": {
      "command": "cmd",
      "args": ["/c", "npx", "-y", "doorframe@0.1.10", "mcp", "--project", "C:\\Users\\alice\\.doorframe\\doorframe.sqlite", "--project-id", "project_123", "--mode", "standard", "--max-results", "25"]
    }
  }
}
```
