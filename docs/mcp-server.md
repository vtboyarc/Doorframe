...
```bash
npx -y doorframe@0.1.12 mcp doctor \
  --project /absolute/path/to/doorframe.sqlite \
  --project-id project_123 \
  --mode standard \
  --max-results 25
...
```json
{
  "mcpServers": {
    "doorframe": {
      "command": "npx",
      "args": ["-y", "doorframe@0.1.12", "mcp", "--project", "/absolute/path/to/doorframe.sqlite", "--project-id", "project_123", "--mode", "standard", "--max-results", "25"]
    }
  }
}
```
...
```bash
npx -y doorframe@0.1.12 mcp --project ./doorframe.sqlite --project-id project_123 --audit-log ./doorframe-mcp-audit.jsonl
```
